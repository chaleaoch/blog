...
语义壁垒, 太难的题AI不会, 让他先思考/推理而不是直接输出答案, AI一着急就会说错\. COT,Chain of Thought,思维链

![image.png](./index/attachments/image.png)

相变, 即使是胡言乱语, 也可能带来意外的惊喜, 但是说错了容易带来前面的\<语义漂移和注意力稀释\>

![image.png](./index/attachments/image6.png)

特征纠缠, 你想让模型增加一个特点，却顺带带出了其他相关特点。譬如,
...
复杂问题, 如果约束太多反而限制AI, 所以可以先发散后收敛

![image.png](./index/attachments/image11.png)

**先推理后结论**, 很多人\(包括我\)喜欢第一句话就给出结论, 后面再扩展, 但是这样的话, 会导致AI后面的扩展全都在圆这个结论而展开, 如果结论给错了就麻烦了\.

![image.png](./index/attachments/image5.png)

入戏与共振采样, 靠连续重复同类关键词，把 AI 锁死在指定人设里，不让它跑偏。

![image.png](./index/attachments/image10.png)

隐式语义优于显式语义
隐式语义 vs 显示语义, **当然这里的显示和隐式是相对的, 否则, 任何指令都可以认为是显示的\.**
...
但是隐式语义会带来一个新的问题, 就是特征纠缠, 所以有需要通过**显示配平**收敛\.
...
引导采样与回滚, 先用一堆话帮模型“想清楚”，拿到准确关键词；然后开新对话，只用这些关键词重新提问。
